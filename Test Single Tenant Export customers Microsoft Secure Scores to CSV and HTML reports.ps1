...
$homePage = "https://NoPiiTest.com"
$appIdURI = "https://3j48zp.onmicrosoft.com/$((New-Guid).ToString())"
$logoutURI = "https://3j48zp.onmicrosoft.com"
...
function New-SITGTableFromArray($Array, $HeaderColour) {
...
function Confirm-MicrosoftGraphServicePrincipal {
    ...
    if (!$graphsp) {
        ...
        New-AzureRmADServicePrincipal -ApplicationId "b5bb42b7-c53a-4bf5-a73f-28ed8f160bac"
        $graphsp = Get-AzureADServicePrincipal -All $true | Where-Object {$_.displayname -eq "Microsoft Graph"}
    }
    return $graphsp
}
   
function Get-SMSGraphResource($Resource) {
...
function Get-AccessToken($appCredential, $tenantId) {
    ...
    foreach ($contract in $contracts) {
        ...
        do {
            try {
                ...
                $access_token = Get-AccessToken -appCredential $appCredential -tenantId $tenant_id
                $headers = @{
                ...
                $scores = Get-MSGraphResource -Resource security/securescores
                $profiles = Get-MSGraphResource -Resource "security/secureScoreControlProfiles"
                ...
                if ($scores) {
                    ...
                    $scoreBreakdownTable = New-SITGTableFromArray -Array $categoryScores -HeaderColour $TableHeaderColour
